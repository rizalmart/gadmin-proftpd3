...
void create_del_system_user_question(struct w *widgets)
{
    ...
    g_signal_connect_swapped(G_OBJECT(question_userdel_cancel_button),
                "clicked", G_CALLBACK(dont_delete_system_user), widgets);

    g_signal_connect(G_OBJECT(question_userdel_delete_button),
            "clicked", G_CALLBACK(do_delete_system_user), widgets);
}
