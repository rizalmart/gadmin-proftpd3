...
void create_security_tab(struct w *widgets)
{
    ...
    gtk_box_pack_start(GTK_BOX(widgets->notebook_vbox6),
            security_treeview_hbox, EXPAND_SECURITY_SECTION, TRUE, 0);
    ...
    widgets->security_store = gtk_list_store_new(6,
            G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

    widgets->security_treeview = gtk_tree_view_new();
    gtk_tree_view_set_model(GTK_TREE_VIEW(widgets->security_treeview), GTK_TREE_MODEL(widgets->security_store));
    ...
    GtkTreeViewColumn *username_col = gtk_tree_view_column_new_with_attributes(
                            _("Security information"), security_cell_renderer, "text", 0, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(widgets->security_treeview), GTK_TREE_VIEW_COLUMN(username_col));


    /* The update file button */
    GtkWidget *security_button_box = gtk_hbutton_box_new();
    gtk_box_pack_start(GTK_BOX(widgets->notebook_vbox6), security_button_box, FALSE, FALSE, 0);
    ...
    g_signal_connect_swapped(G_OBJECT(clear_security_button),
            "clicked", G_CALLBACK(clear_security_tab), widgets);
    ...
    g_signal_connect_swapped(G_OBJECT(update_security_button),
                "clicked", G_CALLBACK(populate_security), widgets);
