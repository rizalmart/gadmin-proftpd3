...
void create_disc_tab(struct w *widgets)
{
    ...
    gtk_box_pack_start(GTK_BOX(widgets->notebook_vbox4), disc_treeview_hbox, EXPAND_DISC_SECTION, TRUE, 0);
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(disc_scrolled_window), GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);

    widgets->disc_store = gtk_list_store_new(6, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
    ...
    gtk_box_pack_start(GTK_BOX(widgets->notebook_vbox4), disc_button_box, FALSE, FALSE, 0);
    ...
    g_signal_connect_swapped(G_OBJECT(update_disc_button), "clicked", G_CALLBACK(populate_discs), widgets);
